...
import { MemoryPressure } from '../types';
export class MemoryUtils {
    /**
     * Format bytes for UI display with proper unit selection
     */
    static formatBytesForUI(bytes) {
     ...
     */
    static parseMemorySize(humanReadable) {
     ...
     */
    static calculateUsagePercentage(snapshot) {
     ...
     */
    static getMemoryPressureColor(pressure) {
        switch (pressure) {
     ...
     */
    static getMemoryPressureFromPercentage(percentage) {
        if (percentage >= 95)
     ...
     */
    static createMemoryUpdate(sessionId, snapshot) {
        return {
            type: "memory_update",
            sessionId,
            snapshot,
    ...
    static validateSessionConfig(config) {
        return (config.sessionId.length > 0 &&
     ...
     */
    static getMemorySizeFromProfile(profile) {
        return profile.val;
    }
    /**
     * Check if memory usage is approaching limits
     */
    static isApproachingLimit(current, limit, thresholdPercent = 90) {
     ...
     */
    static calculateAvailableMemory(total, used) {
     ...
     */
    static calculateEfficiency(used, allocated) {
        if (allocated.bytes === 0n)
            return 1;
     ...
     */
    static isPlatformCompatible(platform, requiredMemory) {
     ...
     */
    static generateMemorySummary(snapshot) {
